...
public abstract class RefinedOilFluid extends FlowableFluid implements GenerateBlockWhenLava{
    ...
    public Item getBucketItem() {
        return ModItems.REFINED_OIL_BUCKET;
    }
    ...
    protected BlockState toBlockState(FluidState state) {
        return (BlockState) AllBlocks.REFINED_OIL.getDefaultState().with(FluidBlock.LEVEL, getBlockStateLevel(state));
    }
...
//            System.out.println(fluidState2+" "+fluidState2.isIn(FluidTags.WATER)+" "+fluidState2.isIn(ModTags.Fluids.REFINED_OIL)+" "+pos+" "+this);
            if (this.isIn(ModTags.Fluids.REFINED_OIL)&&fluidState2.isIn(FluidTags.WATER)&&(!fluidState2.isIn(ModTags.Fluids.REFINED_OIL))) {
    ...
    public static class Still extends RefinedOilFluid{
        @Override
        public int getLevel(FluidState state) {
            return 8;
        }
