...
public abstract class Chlorine extends FlowableFluid {
    @Override
    public Fluid getFlowing() {
        return ModFluids.FLOWING_CHLORINE;
    }

    @Override
    public Fluid getStill() {
        return ModFluids.CHLORINE;
    }
    ...
    public Item getBucketItem() {
        return ModItems.CHLORINE_BUCKET;
    }

    @Override
    protected boolean canBeReplacedWith(FluidState state, BlockView world, BlockPos pos, Fluid fluid, Direction direction) {
        return state.getHeight(world, pos) >= 0.44444445F && (fluid.isIn(FluidTags.WATER) || fluid.isIn(FluidTags.LAVA) ||  fluid.isIn(ModTags.Fluids.MOLTEN_METAL));
    }
    ...
    protected BlockState toBlockState(FluidState state) {
        return AllBlocks.CHLORINE.getDefaultState().with(FluidBlock.LEVEL, getBlockStateLevel(state));
    }
    ...
    public static class Still extends Chlorine {
        @Override
        public int getLevel(FluidState state) {
            return 8;
        }
