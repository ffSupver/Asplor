...
public abstract class Cheese extends FlowableFluid implements GenerateBlockWhenLava{
    @Override
    public Fluid getFlowing() {
        return ModFluids.FLOWING_CHEESE;
    }

    @Override
    public Fluid getStill() {
        return ModFluids.CHEESE;
    }
    ...
    protected BlockState toBlockState(FluidState state) {
        return AllBlocks.CHEESE.getDefaultState().with(FluidBlock.LEVEL, getBlockStateLevel(state));
    }
    ...
    protected void flow(WorldAccess world, BlockPos pos, BlockState state, Direction direction, FluidState fluidState) {
        if (direction == Direction.DOWN) {
            ...
            if (fluidState2.isIn(ModTags.Fluids.MOLTEN_METAL)) {
                if (state.getBlock() instanceof FluidBlock) {
                    world.setBlockState(pos, ModBlocks.MOON_SAND.get().getDefaultState(), 3);;
                }
    ...
    public BlockState getBlockToGenerateWhenLava() {
        return ModBlocks.MOON_SAND.get().getDefaultState();
    }

    public static class Flowing extends Cheese {
        public Flowing() {
        }
    ...
    public static class Still extends Cheese {
        @Override
        public int getLevel(FluidState state) {
            return 8;
        }
